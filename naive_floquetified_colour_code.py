...
# noise_model = CircuitLevelNoise(0.1, 0, 0, 0, 0)
# circuit = naive_memory_experiment(3, 3, 13, noise_model)
...
# dem = circuit.detector_error_model(approximate_disjoint_errors=True, decompose_errors=True)
# print(len(dem.shortest_graphlike_error(ignore_ungraphlike_errors=True,ignore_decomposition_failures=True)))
...
from naive_floquetified_colour_code import NaiveFloquetifiedColourCode

code = NaiveFloquetifiedColourCode(tiles_width=3, tiles_height=3)
coords = sorted(set((int(x), int(y)) for x, y in code.data_qubits))
xs = [c[0] for c in coords]
ys = [c[1] for c in coords]
# print(len(coords), min(xs), max(xs), min(ys), max(ys))
# print(coords)